...
@section('content')
    ...
    <h1>{{ __('permaxis_crudgenerator::messages.edit_entity') }}</h1>

    @if (session('updateAction') && session('updateAction') == 'success')
        <div class="alert alert-success">
            {{ __('permaxis_crudgenerator::messages.entity_updated_with_success') }}
        </div>
    @endif
    @if (session('updateAction') && session('updateAction') == 'failed')
        <div class="alert alert-danger">
            {{ __('permaxis_crudgenerator::messages.entity_updated_not updated') }}
        </div>
    @endif
